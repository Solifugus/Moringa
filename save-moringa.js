...
class Moringa {
	...
	importSupplement( script, name ) {
		...
		var commands = [
			...
			{ command:'retain',       gram:'retain * for * \n',                                param:{ variable:1, for:3 }                },
	...
	actionSay( param, awareness, personality ) {
		let output = this.formatOutput( param.message, awareness.variable, personality.conjugations );
		let time = Date.now();

		if( param.in !== undefined ) time = this.durationToDate( param.in ).getTime(); 
		if( param.at !== undefined ) time = this.SpecificToRawTime( param.at ); 

		// If no time set, respond immediately
		personality.outputs.push( { message:output, time:time } );
	...
	durationToDate( duration, from ) {
		...
		for( var t = 0; t < tokens.length; t += 2 ) {
			...
			switch( measure.toLowerCase() ) {
				case 'year':    case 'years':    from.setYear( from.getYear() + number );       break;
				case 'month':   case 'months':   from.setMonth( from.getMonth() + number );     break;
				case 'week':    case 'weeks':    from.setWeek( from.getWeek() + number );       break;
				case 'day':     case 'days':     from.setDay( from.getDay() + number );         break;
				case 'hour':    case 'hours':    from.setHours( from.getHours() + number );     break;
				case 'minute':  case 'minutes':  from.setMinutes( from.getMinutes() + number ); break;
				case 'second':  case 'seconds':  from.setSeconds( from.getSeconds() + number ); break;
